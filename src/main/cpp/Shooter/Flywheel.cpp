 ...
 * Constructor
*/
Flywheel::Flywheel(std::string name, bool enabled = true, bool shuffleboard = false):
...
void Flywheel::CorePeriodic(){
    double pos = motor_.GetSelectedSensorPosition();
    double vel = motor_.GetSelectedSensorVelocity();
...
void Flywheel::CoreShuffleboardInit(){
    ...
    shuff_.add("kV", &feedforward_.ks, {1,1,1,3}, true);
    shuff_.add("kA", &feedforward_.ks, {1,1,2,3}, true);
};
